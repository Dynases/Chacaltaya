...
#Region "Metodos Privados"
    ...
    Private Sub _prCargarTipoProducto()
        Dim dt As New DataTable
        dt = L_LibreriaGeneral("5").Tables(0)
        With cbTipoProducto
            .DropDownList.Columns.Clear()
            .DropDownList.Columns.Add("cenum").Width = 60
            .DropDownList.Columns("cenum").Caption = "COD"
            .DropDownList.Columns.Add("cedesc").Width = 250
            ...
            .Refresh()
        End With
        If (CType(cbTipoProducto.DataSource, DataTable).Rows.Count > 0) Then
    ...
    Private Sub _prAsignarPermisos()
        ...
        Dim dtRolUsu As DataTable = L_RolDetalle_General2(-1, idRolUsu, "ycyanumi=79")
        ...
        If (dtRolUsu.Rows.Count = 1) Then
            boShow = dtRolUsu.Rows(0).Item("ycshow")
            boAdd = dtRolUsu.Rows(0).Item("ycadd")
            boModif = dtRolUsu.Rows(0).Item("ycmod")
            boDel = dtRolUsu.Rows(0).Item("ycdel")
        End If
    ...
    Public Sub _prMostrarRegistro(_N As Integer)
        ...
        With grmovimiento
            ...
            tbNroNota.Text = .GetValue("ibnota").ToString
            cbTipoProducto.Value = .GetValue("ibtprod")
        End With
    ...
    Private Sub _prCargarVenta()
        ...
        If (tbTablet.Value = True) Then
            dt = L_prMovimientoChoferGeneralSalidaTop20(23)
        Else
            dt = L_prMovimientoChoferGeneralSalida(23)
        End If
        ...
        With grmovimiento.RootTable.Columns("estado")
            ...
            .Visible = False
        End With
        With grmovimiento.RootTable.Columns("ibnota")
            .Width = 100
            .CellStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Far
            .Visible = True
            .Caption = "NOTA"
        End With
        With grmovimiento.RootTable.Columns("ibtprod")
            .Width = 100
            .Visible = False
        End With
        With grmovimiento
    ...
    Private Sub _prCargarProductos()
        Dim dt As New DataTable
        dt = L_prMovimientoChoferListarProductosSalida(CType(grdetalle.DataSource, DataTable), cbTipoProducto.Value.ToString)  ''1=Almacen
    ...
    Public Sub _GuardarNuevo()
        ...
        If (_IdConciliacion = 0) Then
            Dim resconciliacion As Boolean = L_prMovimientoChoferGrabar(numiConciliacion,
                                                                        tbFecha.Value.ToString("yyyy/MM/dd"), 24, "",
                                                                        _codChofer, 0,
                                                                        IIf(tbNroNota.Text.Trim = String.Empty, "0", tbNroNota.Text.Trim),
                                                                        IIf(IsNumeric(cbTipoProducto.Value), cbTipoProducto.Value, "0"))
            If (resconciliacion = False) Then
                ...
                Return
            Else
                Dim tabla As DataTable = L_prMovimientoChoferNoExisteConciliacion(_codChofer, cbTipoProducto.Value.ToString) ''Aqui obtengo el numi de la TI0022 
                Dim res As Boolean = L_prMovimientoChoferGrabarSalida(numi, tbFecha.Value.ToString("yyyy/MM/dd"),
                                                                      cbConcepto.Value, tbObservacion.Text, _codChofer,
                                                                      tabla.Rows(0).Item("ieid"),
                                                                      CType(grdetalle.DataSource, DataTable),
                                                                      IIf(tbNroNota.Text.Trim = String.Empty, "0", tbNroNota.Text.Trim),
                                                                      IIf(IsNumeric(cbTipoProducto.Value), cbTipoProducto.Value, "0"))
                If res Then
                    _prCargarVenta()
        ...
        Else
            Dim res As Boolean = L_prMovimientoChoferGrabarSalida(numi, tbFecha.Value.ToString("yyyy/MM/dd"),
                                                                  cbConcepto.Value, tbObservacion.Text, _codChofer,
                                                                  _IdConciliacion, CType(grdetalle.DataSource, DataTable),
                                                                  IIf(tbNroNota.Text.Trim = String.Empty, "0", tbNroNota.Text.Trim),
                                                                  IIf(IsNumeric(cbTipoProducto.Value), cbTipoProducto.Value, "0"))
            If res Then
                _prCargarVenta()
...
#End Region
    ...
    Public Sub _prObtenerNumiConciliacionTI0022()
        ...
        Dim dt As DataTable = L_prMovimientoChoferExisteConciliacionSalida(_codChofer, cbTipoProducto.Value.ToString) ''Aqui verifico si existe conciliacion con devolucion 
        ...
        If (dt.Rows.Count > 0) Then
            ...
            '    cbConcepto.Value = 9
            'Else
            Dim tabla As DataTable = L_prMovimientoChoferNoExisteConciliacion(_codChofer, cbTipoProducto.Value.ToString) ''Aqui obtengo el numi de la conciliacion si existiera con estado=1 si no tengo que insertar una
...
salirIf:
    ...
    Private Sub P_prAyudaChofer()
        ...
        Dim listEstCeldas As New List(Of Modelos.MCelda)
        listEstCeldas.Add(New Modelos.MCelda("cbnumi,", True, "ID", 50))
        listEstCeldas.Add(New Modelos.MCelda("cbdesc", True, "NOMBRE", 280))
        listEstCeldas.Add(New Modelos.MCelda("cbci", True, "CI".ToUpper, 150))
        listEstCeldas.Add(New Modelos.MCelda("cbfnac", True, "F.Nacimiento", 220, "MM/dd/YYYY"))
