...
Public Class PR_CuentasPagar_PagosRealizados
...
#Region "Metodos"
    ...
    Private Sub P_GenerarReporte()
        ...
        End If
        Dt = L_fnCuentasPagar_ReportePagosRealizados(prov, DtiDesde.Value.ToString("yyyy/MM/dd"), DtiHasta.Value.ToString("yyyy/MM/dd"))

        If (Dt.Rows.Count > 0) Then
            Dim objrep As New R_CuentasPagar_PagosRealizados
            objrep.SetDataSource(Dt)
            objrep.SetParameterValue("FechaIni", DtiDesde.Value.Date.ToShortDateString)
        ...
        Else
            ToastNotification.Show(Me, "No hay pagos realizados para los criterios seleccionados.".ToUpper,
    ...
    Private Function P_Validar() As Boolean
        ...
        If (SbTodosUno.Value) Then
            If (Not IsNumeric(CbProveedor.Value)) Then
                sms = "Elija un proveedor valido."
            End If
        End If
        If (DtiDesde.Value > DtiHasta.Value) Then
            If (sms = String.Empty) Then
                sms = "La fecha ''desde'' debe ser menor o igual a la fecha ''hasta''"
            Else
                sms = sms + Chr(13) + "La fecha ''desde'' debe ser menor o igual a la fecha ''hasta''"
            End If
        End If
