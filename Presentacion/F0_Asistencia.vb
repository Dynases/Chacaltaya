...
#Region "Metodos Privados"
    ...
    Private Sub _prIniciarTodo()
        ...
        TableMarcacion = L_prAsistenciaGeneralReloj() ''En esta linea de codigo lo que hago es solo agarrar la estrucutura y luego los datos lo elimino con el clear como esta abajo
        ...
        If axCZKEM1.ReadGeneralLogData(iMachineNumber) Then 'read all the attendance records to the memory
            ...
            While axCZKEM1.SSR_GetGeneralLogData(iMachineNumber, sdwEnrollNumber, idwVerifyMode, idwInOutMode, idwYear, idwMonth, idwDay, idwHour, idwMinute, idwSecond, idwWorkcode)
                ...
                If (Datanombre.Rows.Count > 0) Then
                    ...
                    TableMarcacion.Rows.Add(iGLCount, sdwEnrollNumber, nombre, DateSerial(idwYear, idwMonth, idwDay).ToString("dd/MM/yyyy"), _fnFormatNumber(idwHour) + ":" + _fnFormatNumber(idwMinute) + ":" + _fnFormatNumber(idwSecond), 0, 0)
                Else
                    TableMarcacion.Rows.Add(iGLCount, sdwEnrollNumber, "USUARIO NO REGISTRADO", DateSerial(idwYear, idwMonth, idwDay).ToString("dd/MM/yyyy"), _fnFormatNumber(idwHour) + ":" + _fnFormatNumber(idwMinute) + ":" + _fnFormatNumber(idwSecond), 0, 0)
    ...
    Private Sub btCargarDatos_Click(sender As Object, e As EventArgs) Handles btCargarDatos.Click
        ...
        If bIsConnected = True Then
            If (TableMarcacion.Rows.Count > 0) Then
                ...
                res = L_prMarcacionGrabar("", CType(grAsistencia.DataSource, DataTable))
                If res Then
...
#End Region
    ...
    Private Sub F0_Asistencia_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        _prDesconectar()
    End Sub
End Class