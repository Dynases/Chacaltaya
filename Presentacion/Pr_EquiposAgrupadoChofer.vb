...
Public Class Pr_EquiposAgrupadoChofer
...
#Region "Eventos"
    ...
    Private Sub Pr_EquiposAgrupadoChofer_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        P_prInicio()
    End Sub
...
#Region "Metodos"
    ...
    Private Sub P_prCargarReporte()
        ...
        If (Not SbFiltroCliente.Value) Then
            'Todos
            cliente = "TODOS"
            dt = L_VistaPrestamoEquiposAgrupadoRepartidor("", where)
        Else
            'Se elijio un cliente
            cliente = TbNombreCliente.Text.Trim
            dt = L_VistaPrestamoEquiposAgrupadoRepartidor(TbCodigoCliente.Text.Trim, where)
        End If

        P_prQuitarProductosCompuesto(dt)

        If (dt.Rows.Count > 0) Then
            Dim objrep As New R_PrestamosEquipoRepartidor
    ...
    Private Sub P_prCargarAyudaCliente()
        If (SbFiltroCliente.Value) Then
            ...
            Dim dt As DataTable = L_GetRepartidor().Tables(0)
            ...
            frmAyuda = New Modelos.ModeloAyuda(750, 400, dt, "Seleccione un Repartidor".ToUpper, listEstCeldas)
            frmAyuda.StartPosition = FormStartPosition.CenterScreen
            frmAyuda.ShowDialog()

            If frmAyuda.seleccionado = True Then
                Dim id As String = frmAyuda.filaSelect.Cells("cbnumi").Value
                Dim descr As String = frmAyuda.filaSelect.Cells("cbdesc").Value
