...
Public Class P_PedidosAsignacion
...
#Region "Metodos Privados"
    ...
    Private Sub _PCargarGridPedidosAtrasados(ByRef objGrid As Janus.Windows.GridEX.GridEX, estado As String, Optional ByVal codZona As String = "", Optional ByVal codRep As String = "-1")
        Dim dtReg As DataTable

        dtReg = L_prObtenerPEdidosConRetrasos(codRep)
        ...
        With objGrid.RootTable.Columns("diasretraso")
            .Visible = True
            .Caption = "Dias de Retraso"
            .Width = 120
        End With

        With objGrid.RootTable.Columns("estado")
            .Visible = True
            .Caption = "EstadoPedido"
            .Width = 100
        End With
    ...
    Private Sub _PCargarGridRegistrosPedidosSoloRepartidor(ByRef objGrid As Janus.Windows.GridEX.GridEX, estado As String, Optional ByVal codZona As String = "", Optional ByVal codRep As String = "-1")
        Dim dtReg As DataTable
        If codZona = "" Then
            If codRep = "-1" Then
                dtReg = L_PedidoCabecera_GeneralSoloRepartidor(-1, " AND (oaest=" + estado + " ) AND oaap=1")
            Else
                dtReg = L_PedidoCabecera_GeneralSoloRepartidor(-1, " AND (oaest=" + estado + " ) AND oaap=1" + " AND tl0012.lccbnumi=" + codRep)
            End If
    ...
    Private Sub _PGrabarAsignaciones()
        ...
        For i = 0 To JGr_Registros1.RowCount - 1
            ...
            If estado = True Then
                ...
                L_PedidoEstados_Grabar(codPedido, "2", Date.Now.Date.ToString("yyyy/MM/dd"), Now.Hour.ToString + ":" + Now.Minute.ToString, gs_usuario)
    ...
    Private Sub _PGrabarConfirmacionesEntregas()
        ...
        For i = 0 To JGr_Registros2.RowCount - 1
            ...
            If estado = True Then
                ...
                L_PedidoEstados_Grabar(codPedido, "3", Date.Now.Date.ToString("yyyy/MM/dd"), Now.Hour.ToString + ":" + Now.Minute.ToString, gs_usuario)
...
#End Region
    ...
    Private Sub AñadirObsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AñadirObsToolStripMenuItem.Click
        ...
        L_PedidoEstados_Grabar(codPedido, "1", Date.Now.Date.ToString("yyyy/MM/dd"), Now.Hour.ToString + ":" + Now.Minute.ToString, gs_usuario)
    ...
    Private Sub GRABARRECLAMOToolStripMenuItem1_Click(sender As Object, e As EventArgs) Handles GRABARRECLAMOToolStripMenuItem1.Click
        ...
        frm.ShowDialog()
        P_prActualizarPendientes()
        P_prActualizarDictados()
    ...
    Public Sub P_prActualizarPendientes()
        If JGr_Zonas1.Row >= 0 Then
            Tb_CodRep1.Text = JGr_Zonas1.CurrentRow.Cells(0).Value
            Tb_Repartidor1.Text = JGr_Zonas1.CurrentRow.Cells(1).Value
            Dim idRegZona As Integer
    ...
    Public Sub P_prActualizarDictados()
        If JGr_Zonas2.Row >= 0 Then
            Tb_CodRep2.Text = JGr_Zonas2.CurrentRow.Cells(0).Value
            Tb_Repartidor2.Text = JGr_Zonas2.CurrentRow.Cells(1).Value
            Dim idRegZona As Integer
    ...
    Private Sub JGr_Zonas4_EditingCell(sender As Object, e As EditingCellEventArgs) Handles JGr_Zonas4.EditingCell
        e.Cancel = True
    End Sub

    Private Sub JGr_Zonas4_SelectionChanged(sender As Object, e As EventArgs) Handles JGr_Zonas4.SelectionChanged
        If JGr_Zonas4.Row >= 0 Then
            swMostrarPedInvalidos.Value = False
            Tb_CodRep4.Text = JGr_Zonas4.CurrentRow.Cells(0).Value
            Tb_Repartidor4.Text = JGr_Zonas4.CurrentRow.Cells(1).Value
            Dim idRegZona As Integer
            Dim color1 As String
            If _soloRepartidor = 0 Then
                idRegZona = JGr_Zonas4.CurrentRow.Cells(2).Value
                Tb_Ciudad4.Text = JGr_Zonas4.CurrentRow.Cells(4).Value
                Tb_Provinicia4.Text = JGr_Zonas4.CurrentRow.Cells(6).Value
                Tb_Zona4.Text = JGr_Zonas4.CurrentRow.Cells(8).Value
                color1 = JGr_Zonas4.CurrentRow.Cells(7).Value
            End If


            If _soloRepartidor = 0 Then
                _PCargarGridPedidosAtrasados(JGr_Registros4, "2", Str(idRegZona), Tb_CodRep4.Text) 'filtrado por repartidor seleccionado en la zona
            Else
                _PCargarGridPedidosAtrasados(JGr_Registros4, "2", , Tb_CodRep4.Text)
            End If
            ''JGr_Registros4.RootTable.Columns("Check").Visible = False 'oculto el check
            JGr_Registros4.ContextMenuStrip = ConMenu_Opciones2 'añadir la opcion para retornar a un estado anterior
        End If
    End Sub
End Class
