...
#End Region
    ...
    Private Sub P_Inicio()
        ...
        If CType(CbFiltroResumenEquipo.DataSource, DataTable).Rows.Count > 0 Then
    ...
    Private Sub P_HabilitarComponentes(ByVal _Bool As Boolean)
        ...
        dgdEquipos.PrimaryGrid.ReadOnly = Not _Bool
    ...
    Private Sub P_ArmarComboCategoria()
        Dim _Ds As New DataSet
        _Ds.Tables.Add(L_CategoriaPrecioGeneral("cinumi>0"))
    ...
    Private Sub P_LlenarDatos(ByVal _index As Integer)
        If (_index <= _MaxReg And _index >= 0 And Gr1Navegacion.Count > 0) Then
            ...
            If (gb_mostrarMapa) Then
                If (Txt_Latitud.Text <> String.Empty And Txt_Longitud.Text <> String.Empty) Then
    ...
    Private Sub P_NuevoRegistro()
        ...
        dgdEquipos.PrimaryGrid.DataSource = DtDetalle
        P_ArmarGrillaSugerencia()
        ...
        If (_Nuevo) Then
            If (P_Validar()) Then
                If (_Grabar = 2) Then
                    ...
                    Dim dt As DataTable = CType(dgdEquipos.PrimaryGrid.DataSource, DataTable).DefaultView.ToTable(True, "chnumi", "chfec", "chcod", "chdesc", "chtmov", "chnrem", "chcan", "chmonbs", "chmonsus", "chnota", "chlin", "estado")
    ...
    Private Sub P_Limpiar()
        ...
        dgdEquipos.PrimaryGrid.Rows.Clear()
        If (gb_mostrarMapa) Then
            _Overlay.Markers.Clear()
            P_MapLoad()
    ...
    Private Sub Btnx_Equipos_Click(sender As Object, e As EventArgs) Handles Bt1AddEquipos.Click
        If (dgdEquipos.PrimaryGrid.Rows.Count = 0) Then
            P_AddFilaEquipos()
            'Dim dt As DataTable = Dgs_Equipos.PrimaryGrid.DataSource
            dgdEquipos.Select()
            dgdEquipos.PrimaryGrid.SetActiveCell(dgdEquipos.PrimaryGrid.GetCell(0, dgdEquipos.PrimaryGrid.Columns("chcod").ColumnIndex))
        Else
            If (P_ValidarFilaGridEquipo(dgdEquipos.PrimaryGrid.Rows.Count - 1)) Then
                P_AddFilaEquipos()
                dgdEquipos.Select()
                dgdEquipos.PrimaryGrid.SetActiveCell(dgdEquipos.PrimaryGrid.GetCell(0, dgdEquipos.PrimaryGrid.Columns("chcod").ColumnIndex))
            End If
    ...
    Private Sub P_AddFilaEquipos()
        ...
        dgdEquipos.PrimaryGrid.Rows.Insert(0, _fil)
    End Sub

    Private Sub dgdEquipos_EndEdit(sender As Object, e As GridEditEventArgs) Handles dgdEquipos.EndEdit
        If (e.GridCell.RowIndex > -1) Then
            'Producto
            If (e.GridCell.ColumnIndex = dgdEquipos.PrimaryGrid.Columns("chcod").ColumnIndex) Then
                G_CodProducto = e.GridCell.FormattedValue.Trim

                Dim _f As GridRow = CType(dgdEquipos.PrimaryGrid.Rows(e.GridCell.RowIndex), GridRow)
                Dim _c As GridCell = CType(_f.Cells("chdesc"), GridCell)
                _c.Value = L_GetIdProductoEquipo("cacod = '" + e.GridCell.FormattedValue.Trim + "'")
            End If

            'Nro Nota
            If (e.GridCell.GridColumn.Name = "chnota") Then
                'Validar que fecha del registro de cliente sea la misma que de la nota
                Dim dt As DataTable = L_GetTabla("TO002",
                                                 "odnumi, odfdoc",
                                                 "odnota=" + e.GridCell.Value.ToString + " and odtprod=1") 'Esta solo para las notas de tipo de producto AGUA
                If (dt.Rows.Count = 1) Then
                    Dim bool As Boolean = (dt.Rows(0).Item("odfdoc") <> dgdEquipos.PrimaryGrid.GetCell(e.GridCell.RowIndex, dgdEquipos.PrimaryGrid.Columns("chfec").ColumnIndex).Value)

                    If (bool) Then
                        Dim info As New TaskDialogInfo("Información Importante".ToUpper,
                                                       eTaskDialogIcon.Information2,
                                                       "Leer con atención".ToUpper,
                                                       "este equipo se esta registrando con una fecha diferente a la del nro de nota ".ToUpper + e.GridCell.Value.ToString _
                                                       + vbCrLf + "se procedera a cambiar la fecha en el equipo con la fecha de la nota".ToUpper,
                                                       eTaskDialogButton.Ok,
                                                       eTaskDialogBackgroundColor.Orange)
                        Dim result As eTaskDialogResult = TaskDialog.Show(info)
                        dgdEquipos.PrimaryGrid.GetCell(e.GridCell.RowIndex, dgdEquipos.PrimaryGrid.Columns("chfec").ColumnIndex).Value = dt.Rows(0).Item("odfdoc")
                    End If
                Else
                    ToastNotification.Show(Me,
                                           "El nro de nota ".ToUpper + e.GridCell.Value.ToString + " no esta registrado.".ToUpper,
                                           My.Resources.WARNING,
                                           5 * 1000,
                                           eToastGlowColor.Red,
                                           eToastPosition.TopCenter)
    ...
    Private Sub Sgc_Equipos_GridPreviewKeyDown(sender As Object, e As GridPreviewKeyDownEventArgs) Handles dgdEquipos.GridPreviewKeyDown
        Dim _col As Integer = dgdEquipos.PrimaryGrid.ActiveCell.ColumnIndex
        Dim _fil As Integer = dgdEquipos.PrimaryGrid.ActiveCell.RowIndex

        If (e.KeyCode <> Keys.Enter And
            (_col = dgdEquipos.PrimaryGrid.Columns("chnrem").ColumnIndex Or
            _col = dgdEquipos.PrimaryGrid.Columns("chcan").ColumnIndex Or
            _col = dgdEquipos.PrimaryGrid.Columns("chmonbs").ColumnIndex Or
            _col = dgdEquipos.PrimaryGrid.Columns("chmonsus").ColumnIndex Or
            _col = dgdEquipos.PrimaryGrid.Columns("chnota").ColumnIndex)) Then
        ...
        ElseIf (_col = dgdEquipos.PrimaryGrid.Columns("chnrem").ColumnIndex) Then
            dgdEquipos.PrimaryGrid.SetActiveCell(dgdEquipos.PrimaryGrid.GetCell(_fil, dgdEquipos.PrimaryGrid.Columns("chcan").ColumnIndex))
            dgdEquipos.PrimaryGrid.SetSelectedCells(_fil, dgdEquipos.PrimaryGrid.Columns("chnrem").ColumnIndex, 1, 1, False)
            dgdEquipos.PrimaryGrid.SetSelectedCells(_fil, dgdEquipos.PrimaryGrid.Columns("chcan").ColumnIndex, 1, 1, True)
            Exit Sub
            'mover a la columna montobs
        ElseIf (_col = dgdEquipos.PrimaryGrid.Columns("chcan").ColumnIndex) Then
            dgdEquipos.PrimaryGrid.SetActiveCell(dgdEquipos.PrimaryGrid.GetCell(_fil, dgdEquipos.PrimaryGrid.Columns("chmonbs").ColumnIndex))
            dgdEquipos.PrimaryGrid.SetSelectedCells(_fil, dgdEquipos.PrimaryGrid.Columns("chcan").ColumnIndex, 1, 1, False)
            dgdEquipos.PrimaryGrid.SetSelectedCells(_fil, dgdEquipos.PrimaryGrid.Columns("chmonbs").ColumnIndex, 1, 1, True)
            Exit Sub
        ...
        ElseIf (_col = dgdEquipos.PrimaryGrid.Columns("chmonbs").ColumnIndex) Then
            dgdEquipos.PrimaryGrid.SetActiveCell(dgdEquipos.PrimaryGrid.GetCell(_fil, dgdEquipos.PrimaryGrid.Columns("chnota").ColumnIndex))
            dgdEquipos.PrimaryGrid.SetSelectedCells(_fil, dgdEquipos.PrimaryGrid.Columns("chmonbs").ColumnIndex, 1, 1, False)
            dgdEquipos.PrimaryGrid.SetSelectedCells(_fil, dgdEquipos.PrimaryGrid.Columns("chnota").ColumnIndex, 1, 1, True)
            Exit Sub
        End If

        'mover foco a la siguiente columna
        If (e.KeyCode = Keys.Enter And
            Not (_col = dgdEquipos.PrimaryGrid.Columns("chnrem").ColumnIndex Or
            _col = dgdEquipos.PrimaryGrid.Columns("chcan").ColumnIndex Or
            _col = dgdEquipos.PrimaryGrid.Columns("chmonbs").ColumnIndex Or
            _col = dgdEquipos.PrimaryGrid.Columns("chmonsus").ColumnIndex Or
            _col = dgdEquipos.PrimaryGrid.Columns("chnota").ColumnIndex)) Then
            e.Handled = True
            SendKeys.Send("{TAB}")
            If (_col = dgdEquipos.PrimaryGrid.Columns("chcod").ColumnIndex) Then
                e.Handled = True
                SendKeys.Send("{TAB}")
                SendKeys.Send("{F2}")
            End If
    ...
    Private Function P_ValidarFilaGridEquipo(_f As Integer) As Boolean
        ...
        Dim s As String = dgdEquipos.PrimaryGrid.GetCell(_f, dgdEquipos.PrimaryGrid.Columns("chcod").ColumnIndex).FormattedValue.ToString.Trim
        If (dgdEquipos.PrimaryGrid.GetCell(_f, dgdEquipos.PrimaryGrid.Columns("chcod").ColumnIndex).FormattedValue.ToString.Trim.Equals("")) Then
        ...
        If (dgdEquipos.PrimaryGrid.GetCell(_f, dgdEquipos.PrimaryGrid.Columns("chtmov").ColumnIndex).FormattedValue.ToString.Trim.Equals("")) Then
        ...
        If (dgdEquipos.PrimaryGrid.GetCell(_f, dgdEquipos.PrimaryGrid.Columns("chcan").ColumnIndex).Value.ToString.Trim.Equals("0")) Then
    ...
    Private Sub P_PonerDatosEquipoPorCliente(_CodCli As String)
        ...
        If (DtDetalle.Rows.Count > 0) Then
            dgdEquipos.PrimaryGrid.DataSource = DtDetalle
        Else
            dgdEquipos.PrimaryGrid.DataSource = Nothing
        End If
    End Sub
    ...
    Private Sub Tsmi1Eliminar_Click(sender As Object, e As EventArgs) Handles Tsmi1Eliminar.Click
        Dim fi As Integer = dgdEquipos.PrimaryGrid.ActiveRow.RowIndex
        If (fi > -1) Then
            If (_Nuevo) Then
                dgdEquipos.PrimaryGrid.Rows.RemoveAt(fi)
            ElseIf (_Mofificar) Then
                dgdEquipos.PrimaryGrid.GetCell(fi, dgdEquipos.PrimaryGrid.Columns("estado").ColumnIndex).Value = -1
                dgdEquipos.PrimaryGrid.Rows(fi).Visible = False
            End If
        End If
    End Sub
    ...
    Private Sub Dgs_Equipos_CellValueChanged(sender As Object, e As GridCellValueChangedEventArgs) Handles dgdEquipos.CellValueChanged
        Dim cel As GridCell = dgdEquipos.PrimaryGrid.GetCell(e.GridCell.RowIndex, dgdEquipos.PrimaryGrid.Columns("estado").ColumnIndex)
        If (cel.Value = 1) Then
            cel.Value = 2
        End If
    End Sub
    ...
    Private Function P_ResumenProducto(cpro As String, sr As String) As Double
        Dim dt As DataTable
        Dim sum As Double = 0
        dt = CType(dgdEquipos.PrimaryGrid.DataSource, DataTable)
