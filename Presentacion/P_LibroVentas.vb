...
#Region "Variables Globales"
    ...
    Dim lsRutaLCV As String = gs_CarpetaRaiz + "\LCV"
...
#Region "Metodos"
    ...
    Private Sub P_Inicio()
        ...
        '
        Me.Text = "L I B R O S   D E   V E N T A S"
...
#End Region
    ...
    Private Sub P_ComboRazonSocial()
        ...
        For Each _f As DataRow In _Dt.Rows
            _item = New ComboItem()
            _item.Text = _f.Item("scnom").ToString
    ...
    Private Sub P_LlenarDatosGrilla()
        _DsLV = New DataTable
        _DsLV = L_ObtenerLV(Tb1CodRazonSocial.Text, CStr(Cb2Meses.SelectedIndex + 1), Cb1Anho.Text)
    ...
    Private Sub P_GenerarReporte()
        ...
        _Ds1 = Nothing 'L_ObtenerRutaImpresora("1") ' Datos de Impresion
        Dim rutaImp As String = "PDF24"
        'If (CInt(_Ds1.Tables(0).Rows(0).Item("cgvp").ToString) = 1) Then 'Vista Previa de la Ventana de Vizualización 1 = True 0 = False
        P_Global.Visualizador = New Visualizador 'Comentar
        'End If

        Dim objrep As New R_LV
        ...
        pd.PrinterSettings.PrinterName = rutaImp '_Ds1.Tables(0).Rows(0).Item("cgrut").ToString
        If (Not pd.PrinterSettings.IsValid) Then
            ToastNotification.Show(Me, "La Impresora ".ToUpper + rutaImp + Chr(13) + "No Existe".ToUpper,
                                   My.Resources.INFORMATION, _DuracionSms * 1000,
                                   eToastGlowColor.Blue, eToastPosition.BottomLeft)
        Else
            objrep.PrintOptions.PrinterName = rutaImp '"EPSON TM-T20II Receipt5 (1)"
            objrep.PrintToPrinter(1, False, 1, 1)
        End If

        'Preguntando si existe el Directorio
        If (Not Directory.Exists(lsRutaLCV)) Then
            Directory.CreateDirectory(lsRutaLCV)
        End If
        'Exportando a Excel el Reporte
        objrep.ExportToDisk(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, lsRutaLCV + "\LCV_" + Cb3RazonSocial.Text + ".pdf")
    End Sub
