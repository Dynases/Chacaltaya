...
Public Class AccesoDatos
    ...
    Public Shared Function D_ProcedimientoConParam(_Nombre As String, _listaParam As List(Of DParametro)) As DataTable
        ...
        For i = 0 To _listaParam.Count - 1
            ...
            Dim tipo As String = _listaParam.Item(i).tipo
            Dim tam As Int16 = _listaParam.Item(i).tamano
            ...
            Select Case tipo
                ...
                Case "bit"
                    tipoSql = SqlDbType.Bit
                Case "float"
                    tipoSql = SqlDbType.Float
            End Select

            If tam = -1 Then
                _comandoProcedimiento.Parameters.Add(nombre, tipoSql)
            Else
                _comandoProcedimiento.Parameters.Add(nombre, tipoSql, tam)
            End If
            _comandoProcedimiento.Parameters(nombre).Value = valor
        Next

        Return MetodoDatos.EjecutarProcedimiento(_comandoProcedimiento)
    End Function

    Public Shared Function D_ProcedimientoConParam2(_Nombre As String, _listaParam As List(Of DParametro)) As DataTable
        _comandoProcedimiento.Parameters.Clear()
        _comandoProcedimiento.CommandText = _Nombre
